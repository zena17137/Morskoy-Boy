class OnlineScene extends Scene {
	...
	init() {
		...
		socket.on('challengeOpponent', (key) => {
			...
			alert(`Перший хто пройде по цій ссилці буде грати з вами:\n${location.href}`);
		});
	...
	statusUpdate() {
		...
		if (!this.status) {
			...
			statusDiv.textContent = 'Ви виграли!';
		} else if (this.status === 'loser') {
			statusDiv.textContent = 'Ви програли';
		} else if (this.status === 'waiting') {
			statusDiv.textContent = 'Чекаєм суперника';
		}
	}
